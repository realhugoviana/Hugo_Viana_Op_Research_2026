...
void add_arc(Graph* graph, int from, int to, int min_capacity, int capacity, int cost) {
    Arc* forward_arc = allocate_arc(graph, from, to, min_capacity, capacity, cost, 0);
    Arc* backward_arc = allocate_arc(graph, to, from, 0, 0, -cost, 0);
...
Graph* read_graph_stdin() {
    ...
    for (int i = 0; i < num_arcs; i++) {
        ...
        add_arc(graph, from, to, 0, capacity, cost);
    }
    return graph;
}
